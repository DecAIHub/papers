...
**Evaluating Sparse and Dense Sentence Encoders for Vocabulary Governance in Emerging-Domain Taxonomies: A Cross-Family Robustness Approach**

Nikita N. Kadochnikov  
Target journal: *Natural Language Processing Journal*
...
```
taxonomy-vocabulary-governance/
├── README.md
├── LICENSE                        CC BY 4.0
├── requirements.txt               Python dependencies
├── data/
│   ├── llm_validation_results.csv Zero-shot LLM validation outputs (artifact 7)
...
│       └── LICENSE-MOVIELENS.txt  Original GroupLens license
...
| 2 | Normalized value vocabularies | `data/tables/clusters_*.csv` | Deduplicated value lists for `ai_component` and `use_case` with frequency counts and frequency-class assignments (head/mid/tail) |
| 3 | Pairwise similarity outputs | `data/similarity/pairs_*.csv` | Top-50 neighbor pairs per operationalization (Ops A–C); top-100 for Ops D–E as extended scaling-check data referenced in §4.2.2 / Supplementary, for both fields, including cosine similarity scores and surface-consistency classifications |
...
| 6 | Reproducibility code | `scripts/` | Python script reproducing all figures from raw data when the project-card corpus is provided, or fig1 + fig7 from shared artifacts otherwise; fixed random seeds (seed = 42) |
| 7 | Zero-shot LLM validation | `data/llm_validation_results.csv` | Per-pair human-vs-LLM judgments on the validation subsample (n = 66 pairs: 16 cross-family-stable (LLM-evaluable subset of the 27 adjudicated stable pairs from §4.3.1) + 50 negative controls). Source data for the LLM-comparison results reported in the abstract and §4 |
...
| File | Description |
|------|-------------|
| `llm_validation_results.csv` | Per-pair zero-shot LLM judgments aligned with human gold labels. Columns: `pair_id`, `field` (`ai_component` or `use_case`), `value_a`, `value_b`, `stratum` (`cross_family_stable` or `negative_control`), `human_is_synonym` (0/1, adjudicated gold), `llm_answer` (raw model reply: `yes`/`no`), `llm_is_synonym` (0/1 derived from `llm_answer`). Used to compute the headline LLM sensitivity figure (3/16 = 18.8% on cross-family-stable pairs) and the LLM precision/recall numbers in §4 |
...
| `robustness_ai_component.csv` | Table 7 — Within- vs. cross-family overlap at multiple k (ai_component) |
| `robustness_use_case.csv` | Table 7 — Within- vs. cross-family overlap at multiple k (use_case) |
...
The zero-shot LLM judgments in `data/llm_validation_results.csv` were obtained with default sampling parameters (no temperature override) and are not regenerated by the figure-generation script; the file is included as primary evidence for the LLM-comparison results.

## Citation

If you use this replication package, please cite the manuscript:

> Kadochnikov NN (2026) Evaluating Sparse and Dense Sentence Encoders for Vocabulary Governance in Emerging-Domain Taxonomies: A Cross-Family Robustness Approach. *Natural Language Processing Journal*.
